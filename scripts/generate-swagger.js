...
const options = {
  ...
  apis: [path.resolve(__dirname, '../server/routes.ts'), path.resolve(__dirname, '../server/services/*.ts')]
};

const specs = swaggerJSDoc(options);

fs.ensureDirSync(path.resolve(__dirname, '../dist'));
fs.writeFileSync(path.resolve(__dirname, '../dist/swagger.json'), JSON.stringify(specs, null, 2));
